...
Sadly we see that the weak instrument problem has not been resolved and likely been made worse by adding an additional weak instrument (which we suspect *lcigpric* to be since the F-stat is now lower for both regression specifications). Naturally since this additional instrument does not result in any more explanatory power for the treatment by the instruments, we see virtually no change in the point estimates from IV for $\beta_1$ and its standard errors when using both *restaurn* and *lcigpric* as instruments for *cigs*. As a result, our conclusions from part (g) do not change. Furthermore, we test overidentifying restrictions below and find that **Check for overidentifying restrictions?**
...
In both cases we fail to reject the null that the instruments are exogenous, indicating that the regressions do not suffer from overidentification restrictions. 
