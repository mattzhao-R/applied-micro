...
tableII <- cbind(t2_means,(rbind(t2_math_pre_robust_clustered,t2_verb_pre_robust_clustered,t2_math_post_robust_clustered,t2_verb_post_robust_clustered) %>% filter(term == 'bal') %>% select(!c(term,statistic,p.value)))) %>% mutate(across(where(is.numeric), round, 4))
tableII
```
