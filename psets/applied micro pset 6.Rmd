...
In this setting we would say that the DiD estimate from part e is the most credible. This is because the identifying assumptions for the other two estimates are not credible since we see time effects and that the two groups have different average outcomes in the pre-period. **can we show parallel trends with onyl pre and post periods?**
...
# coeftest(lm.parthiii,
#          vcov = vcovCL,
#          type = "HC1",
#          cluster = ~highearn)
```

The estimate is of similar magnitude but the standard errors are much larger resulting in the estimate not being statistically significant. **what od we make of this**
