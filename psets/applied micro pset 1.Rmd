...
$$ lwage_i = \beta_0 + \beta_1 HS_i + \beta_2 College_i + \beta_3 KWW_i + \beta_4 exper_i + \beta_5 exper_i * HS + \beta_6 exper_i * College + U_i$$
...
$$ lwage_i = \beta_0 + \beta_1 educ_i + \beta_3 KWW_i + \beta_4 exper_i + \beta_5 black_i + U_i (1) $$

$$ lwage_i = \beta_0 + \beta_1 HS_i + \beta_2 College_i + \beta_3 KWW_i + \beta_4 exper_i + \beta_5 black_i + U_i$$

#### (ii)

```{r}
m1 <- lm(lwage ~ educ + HS + College + KWW + black, data = df)
m2 <- lm(lwage ~ educ*HS + educ*College + KWW + black, data = df)

models <- list(m1,m2)
stargazer(models,type='text',digits=3,
          title='OLS Estimates including race')
```
...
**what hypothese are we testing?**

### k)

#### (i)

$$ lwage_i = \beta_0 + \beta_1 educ_i + \beta_3 KWW_i + \beta_4 exper_i + \beta_5 black_i + \beta_6 black_i * educ_i + U_i (1) $$
$$ lwage_i = \beta_0 + \beta_1 HS_i + \beta_2 College_i + \beta_3 KWW_i + \beta_4 exper_i + \beta_5 black_i  + \beta_6 black_i * educ_i + U_i$$

#### (ii)

```{r}
m1 <- lm(lwage ~ educ + HS + College + KWW + black + black * educ, data = df)
m2 <- lm(lwage ~ educ*HS + educ*College + KWW + black + black * educ, data = df)
...
```{r}
m1 <- lm(lwage ~ educ + HS + College + KWW + black + black * educ + south, data = df)
m2 <- lm(lwage ~ educ*HS + educ*College + KWW + black + black * educ + smsa, data = df)
m <- lm(lwage ~ educ*HS + educ*College + KWW + black + black * educ + smsa, data = df)


models <- list(m1,m2,m3)
stargazer(models,type='text',digits=3,
          title='Sensitivity Test to Location')
```
